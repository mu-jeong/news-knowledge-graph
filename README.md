...
> 주의: 네이버 뉴스 Search API 문서 기준으로 `start` 파라미터는 최대 `1000`까지만 지원됩니다. 따라서 현재 문서 해석상 안전하게 탐색 가능한 범위는 최대 약 `1000건`이며, 실제 수집 가능 기간은 요청 일수보다 최신 검색 결과 약 1000건의 마지막 기사 날짜에 의해 제한될 수 있습니다. 기사량이 많은 키워드는 최근 며칠치만 수집될 수 있습니다.

> 주의: 뉴스 검색 및 LLM 호출 사용량에 따라 수집기간 1일당 약 100원 수준의 검색 비용이 발생할 수 있습니다.
...
선택한 날짜 범위를 기준으로, 해당 기간의 핵심 뉴스 흐름을 빠르게 요약해 보여줍니다.  
그래프를 먼저 훑고 나서 세부 기사까지 내려가기 전에, 기간 전체의 분위기와 주요 이슈를 압축해서 파악할 수 있도록 만든 기능입니다.

![기간별 주요 뉴스 요약](examples/summary.png)
*지정한 기간 안에서 중요한 뉴스 맥락을 먼저 요약해 보여주기 때문에, 그래프 탐색과 질의응답 전에 큰 흐름을 빠르게 파악할 수 있습니다.*

---

### 7️⃣ 투자자 Q&A 스타일 질의응답

Graph RAG 챗봇을 투자자 관점의 질문 흐름에도 바로 활용할 수 있습니다.  
실적, 리스크, 공급망, 수요 변화, 경쟁 구도처럼 투자 판단에 자주 쓰이는 질문을 던지면, 그래프와 기사 근거를 바탕으로 답변을 정리합니다.

![투자자 Q&A 예시 1](examples/investor_qna.png)
*기업 영향, 수혜/피해, 연관 리스크를 투자자 시각으로 바로 물어보고 근거 기사와 함께 답변받는 예시입니다.*

![투자자 Q&A 예시 2](examples/investor_qna2.png)
*질문을 더 구체화해도 동일한 그래프/기사 기반으로 논리를 이어가며, 투자자용 리서치 메모처럼 활용할 수 있습니다.*
