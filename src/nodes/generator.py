...
def generator_node(state: AgentState) -> dict:
    ...
    [중요 가이드라인]
    1. 각 문장이나 절 뒤에 반드시 `[출처: 기사 ID]` (예: [출처: Article_1])를 표기하여 근거를 명시하세요.
    2. 답변 맨 아래에 "🔗 **참조 뉴스 링크**" 섹션을 만드세요.
    3. **내용에 실제로 인용된(사용된) 기사의 링크만** 중복 없이 나열하세요. 관련 없는 기사 링크는 모두 제외하여 답변의 정밀도를 높이세요.
