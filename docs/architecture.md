...
5. **Entity Resolution (정규화):** 추출된 엔티티의 동의어를 `entity_aliases.json`으로 표준화하고, `entity_taxonomy.json` 기반 taxonomy 및 선택적 임베딩 기반 semantic merge를 적용합니다. 관계 provenance(`article` / `taxonomy`)도 함께 보존합니다.
...
* **`entity_taxonomy.json`:** canonical 엔티티, 별칭, 상위 taxonomy 관계(`PART_OF`, `BELONGS_TO`)를 외부 JSON 파일로 관리합니다.
...
* **`entity_resolution.py`:** 데이터의 파편화를 막기 위한 핵심 모듈입니다. '삼전', '삼성물산', 'Samsung' 등 다양한 형태로 등장하는 엔티티를 하나의 일관된 표준어로 정규화합니다.
  * alias 기반 정규화(`entity_aliases.json`)와 taxonomy 기반 canonical entity 확장(`entity_taxonomy.json`)을 함께 수행합니다.
